	...
	apply(compiler) {
		...
		compiler.hooks.compilation.tap(
			...
			(compilation, { normalModuleFactory }) => {
				...
				const getLockfile = cachedWithoutKey(callback => {
					const readLockfile = () => {
				...
				for (const { scheme, fetch } of schemes) {
					const resolveContent = (url, integrity, callback) => {
						const handleResult = (err, result) => {
							...
							if ("location" in result) {
								// TODO handle validUntil
								return resolveContent(result.location, integrity, callback);
							} else {
								if (
									!result.fresh &&
									integrity &&
					...
					const fetchContentRaw = (url, cachedResult, callback) => {
						const finishWith = result => {
							cache.store(url, null, result, err => {
								if (err) {
									logger.warn(`${url} can be stored in cache: ${err.message}`);
							...
							getLockfile((err, lockfile) => {
								...
								const doFetch = lockedContent => {
									resolveContent(url, entry.integrity, (err, result) => {
										...
										if (
											!areLockfileEntriesEqual(result.entry, entry) ||
											(!lockedContent && cacheLocation)
										) {
